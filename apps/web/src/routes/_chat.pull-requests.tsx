...
import { HeaderFilterMenu, type HeaderFilterMenuOption } from "../components/HeaderFilterMenu";
import {
  WorkspaceBreadcrumb,
...
/** Adapts pull-request filter metadata to the shared header menu. */
function CompactFilterMenu<Value extends string>({
  ...
  const headerOptions = useMemo<ReadonlyArray<HeaderFilterMenuOption<Value>>>(
    () =>
      options.map((option) => ({
        value: option.value,
        label: option.label,
        icon: <option.Icon aria-hidden className="size-3.5" />,
        ...(option.unavailable !== undefined ? { disabledReason: option.unavailable } : {}),
      })),
    [options],
  );
  return (
    <HeaderFilterMenu label={label} value={value} options={headerOptions} onChange={onChange} />
  );
}
