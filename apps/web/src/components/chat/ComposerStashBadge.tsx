...
export const ComposerStashBadge = memo(function ComposerStashBadge(props: {
  ...
  return (
    <button
      ...
      className={cn(
        "inline-flex cursor-pointer items-center gap-1.5 border leading-none",
        props.embedded
          ? "ms-auto h-6 shrink-0 rounded-md px-2 text-[11px]"
          : "chat-composer-stash-tab absolute -top-7 right-4 z-0 h-8 rounded-t-xl border-b-0 px-3 pb-1 text-xs",
        "transition-[color,border-color] duration-200",
        props.embedded && (props.pulsing ? "border-border" : "border-border/70"),
